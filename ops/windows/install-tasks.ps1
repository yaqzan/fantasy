# Registers the "Fantasy Watchdog" scheduled task (every 5 min).
...
try {
    ...
    Register-ScheduledTask -TaskName 'Fantasy Watchdog' -Action $action `
        -Trigger $triggers -Settings $settings -Force -ErrorAction Stop | Out-Null
    Write-Host 'Registered task: Fantasy Watchdog (every 5 min + at logon)'
} catch {
    Write-Host "Register-ScheduledTask failed ($($_.Exception.Message.Trim())) - falling back to schtasks"
    schtasks /create /tn "Fantasy Watchdog" /sc minute /mo 5 /tr "wscript.exe $argString" /f
    if ($LASTEXITCODE -ne 0) { throw "schtasks fallback failed with exit code $LASTEXITCODE" }
    Write-Host 'Registered task: Fantasy Watchdog (every 5 min, current user)'
}
